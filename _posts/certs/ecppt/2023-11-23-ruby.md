...
![Alt text](/assets/images/posts/2023-11-19-ecppt/ruby.jpg){: .align-center}
...
name ="john"
 → "My name is #{name}" // just like python fstring
```
#### Some useful methods
...
print_name // <anything as parameter>
	unknown // <the method grab the default value>
```
...
### tricks
We can declar multiple variables:
```ruby
   → a,b,c = "a","b","c"
```
...
```ruby
t = Time.new.utc
	2014-01-13 10:42:47 UTC
t.zone
	"UTC"
t.localtime
	2014-01-13 11:42:47 +0100
t.zone
	"CET"
...
// an array
t.to_a
	[39, 57, 11, 13, 1, 2014, 1, 13, false, "CET"]
...
Dir.entries(".") // le dot means the current folder
	['chdir.rb', '.', '..', 'chdir-rb~', 'nested_dir', 'test']
```
...
stream.each do |line|            # for each line of the stream
	line.match(ip_pattern) do # if the ip is found and the ip host is up we have to extract each port
		stream.readline.match up_host_pattern do
			...
			stream.each do |line| # from here each line stores the port
				...
				break if line == "\n" # if all the lines have been parsed -> exit from block
			end
		end
	end
end
...
def main(host, start_port,end_port)
	...
	puts "OPEN" if !open.empty?
	puts open if !open.empty?
	puts "FILTERED" if !filtered.empty?
	puts filtered if !filtered.empty?
end
...
def main(host,ports)
	...
	ports.each do |port|
		#send udp packets to current port
	end
	puts "OPEN" if !open.empty?
	puts open if !open.empty?
	puts "FILTERED|OPEN" if !filtered.empty?
	puts filtered if !filtered.empty?
	puts "CLOSED" if !closed.empty?
	puts closed if !closed.empty?
end
...
follow_chain(URI(Target)) { |url,resp|
	case resp
...
```ruby
opt.class.name
"Net:HTTPMethodNotAllowed" // does not allowed the options verb
```
...
class Metasploit4 < Msf::Exploit::Remote  //module type
	include Exploit::Remote::Tcp // module requirements
	def initialize(info = {}) // module information
...
def initialize(info = {})
	super(update_info(info,
		...
		'Description' => %q{
			This module exploits a bof found in the Els ECHO Server.
		},
...
```ruby
socks.gets() // to get the banner sent by the server
```
...
### sys power
```ruby
client.sys.power.shutdown // shutdown the victim machine
```

### scraper
In meterpreter:
```ruby
run scraper -h // get system info including network shares, registry hives and password hashes
```
